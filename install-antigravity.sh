...
if [[ -n "${SELF_DIR}" && -f "${SELF_DIR}/scripts/install-antigravity.py" ]]; then
  exec python3 "${SELF_DIR}/scripts/install-antigravity.py" "$@"
fi
...
if [[ -z "${ARCHIVE_ROOT}" || ! -f "${ARCHIVE_ROOT}/scripts/install-antigravity.py" ]]; then
  printf 'Could not find scripts/install-antigravity.py in archive from %s\n' "${ARCHIVE_URL}" >&2
  exit 1
fi

exec python3 "${ARCHIVE_ROOT}/scripts/install-antigravity.py" \
