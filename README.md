...
`knowledge/README.md` is the contract for persistent business intelligence.

Key rules:

- **Flat organization**: No project slug nesting so agents can find relevant knowledge across all projects.
- **Deliverable headers**: Every file starts with YAML frontmatter (`role`, `project`, `run_id`, `confidence`, `inputs_used`, `evidence_mode`).
- **Lossless history**: When updating a deliverable, agents write to `knowledge/runs/<run-id>/` first, then may update the canonical file. Previous versions are never overwritten.
- **Knowledge continuity**: Before every assignment, the orchestrator scans `knowledge/` for relevant prior deliverables and includes them in `reads_from`. Decisions compound across projects.
