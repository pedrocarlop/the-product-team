#!/usr/bin/env python3
"""Install the Product Team workflow for Claude Code into a target project."""
...
def managed_claude_block(fragment: str) -> str:
    fragment = fragment.strip()
    return f"{MARKER_START}\n{fragment}\n{MARKER_END}\n"


def update_claude_md(fragment_path: Path, target_root: Path) -> None:
    claude_path = target_root / TARGET_FILE
    managed_block = managed_claude_block(fragment_path.read_text(encoding="utf-8"))

    if not claude_path.exists():
        claude_path.write_text(managed_block, encoding="utf-8")
        return
    ...
    claude_path.write_text(updated.rstrip() + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    update_claude_md(root / "assets" / FRAGMENT_NAME, target_root)
    write_manifest(target_root, roles, install_source)

    print(f"Installed Product Team v{PACKAGE_VERSION} for Claude Code into {target_root}")
