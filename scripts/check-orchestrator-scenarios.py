...
def main() -> int:
    ...
    expect("Do not delete detail from specialist plans during merge" in orchestrator_prompt, "Orchestrator prompt missing no-detail-deletion rule.", failures)
    ...
    expect("Role-local skills consulted" in staff_skill, "Staff skill missing skills-consulted role-plan rule.", failures)
    ...
    expect("Skill sources read" in reconcile_skill, "Reconcile skill missing skill-source output.", failures)
    ...
    for path in specialist_files:
      ...
      expect("Role-local skills consulted" in prompt, f"{role}: missing skills-consulted planning section.", failures)
    ...
    expect("Overlap resolutions and conflict decisions" in logs_readme, "logs/README.md missing overlap-resolution contract.", failures)
