<!-- PRODUCT_TEAM_FOR_ANTIGRAVITY:START -->
...
- **Logging & Knowledge Fallback**: If the `logs` or `knowledge` MCP is missing or fails (common in early or non-Codex installs), you **must** use direct filesystem tools (`write_to_file`, `replace_file_content`) to create and update these records in the required `/logs` and `/knowledge` structure. Do not skip logging even if the specialized server is absent.
...
<!-- PRODUCT_TEAM_FOR_ANTIGRAVITY:END -->
