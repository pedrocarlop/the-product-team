...
- **Lossless Handoff**: Prioritize fidelity over conciseness. Specialists must produce at least one standalone deliverable per assigned skill (formatted as `knowledge/<role>-<skill>.md`).
...
- **LLM Wiki Architecture**: The knowledge base is a compounding wiki of synthesized markdown in `knowledge/`. Every task must produce its deliverable in `knowledge/`. Good answers must be filed back into the wiki. Appending to `knowledge/log.md` is mandatory for all changes. Cross-cutting concepts (competitors, personas, patterns) get dedicated entity pages in `knowledge/entities/`.
...
- **Output Routing**: Intelligence outputs (research, strategies, designs, deliverables) go to `knowledge/`. Code outputs go to `app/`. Execution records (prompts, reasoning traces, staffing contracts, routing decisions) go to `logs/`.
- For greenfield product work, `ui-designer` seeds `knowledge/project-ds-spec.md` from up to 3 inspiration-only company references in `.claude/product-team/references/reference-design-systems/`; `design-systems-designer` then operationalizes that shared spec for tokens, components, widget layouts, governance, and QA.
