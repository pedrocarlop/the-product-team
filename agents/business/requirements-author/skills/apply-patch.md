...
name: apply-patch
tool: apply_patch
description: Write and iterate on the Requirements Author plan and deliverable artifacts inside `/logs`.
---

# Apply Patch

Use this skill to write the Requirements Author's plan and deliverable artifacts inside `logs/active/<project-slug>/`. This role now records both its working plan and its authored requirements package in `/logs`.

## When to Use

- After the orchestrator has staffed the Requirements Author and the role has accepted ownership
- When drafting `plans/requirements-author.md` before any substantial execution
- When iterating on `deliverables/requirements-author.md` in response to review findings or new constraints

## How to Use

Invoke `apply_patch` targeting one of these files:

- `logs/active/<project-slug>/plans/requirements-author.md`
- `logs/active/<project-slug>/deliverables/requirements-author.md`
