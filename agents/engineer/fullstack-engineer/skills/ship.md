...
Use this skill to turn an implemented full-stack feature into a safe production release with verification, monitoring, and rollback readiness.

## When to Use

- When code is complete and the remaining work is validation or rollout
- When a feature flag, migration, or deployment sequence needs coordination
...
- The implemented API, UI, and data changes
- Test results, screenshots, or validation evidence
- Feature flag, migration, and deployment constraints
- Observability and rollback requirements

## Workflow

1. Verify the implemented behavior against the intended model and wireframe.
2. Confirm tests cover the happy path, key failure modes, and any auth boundaries.
3. Check that rollout dependencies such as flags, migrations, and deploy order are safe.
...
- Rollout, flag, migration, and rollback notes
